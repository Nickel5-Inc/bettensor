...
"""
Impact analysis for ROI-based tier demotion across multiple timeframes.
...
def analyze_multi_window_roi_impact(db_path, print_miners=False):
    """
    Analyze the impact of demoting miners if they have negative ROI in any timeframe.
    ...
    """
    logger.info(f"Analyzing multi-window ROI impact using database at {db_path}")
    ...
    try:
        ...
        for miner_uid in unique_miners:
            ...
            # Also store lifetime ROI from miner_stats
            windows['lifetime'] = {
                'roi': None,  # We'll fill this from miner_stats
                'days': len(sorted_miner_scores)
            }
        ...
        for miner_uid, tier, lifetime_roi in miners:
            ...
            if miner_uid in miner_roi_windows:
                miner_roi_windows[miner_uid]['lifetime']['roi'] = lifetime_roi
        ...
        if print_miners and miners_to_demote:
            logger.info("\nMiners to demote (having negative ROI in at least one timeframe):")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Analyze impact of multi-window ROI-based tier demotion")
    parser.add_argument(
        "--db-path", 
        type=str, 
    ...
    try:
        analyze_multi_window_roi_impact(db_path, args.print_miners)
    except Exception as e:
