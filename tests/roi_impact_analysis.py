#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Impact analysis for ROI-based tier demotion.
This script analyzes existing miner data to estimate how many miners would be
demoted from each tier if the ROI-based tier requirements were enforced.
"""

import argparse
import logging
import os
import sys
import sqlite3
import numpy as np
from datetime import datetime, timedelta, timezone

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[logging.StreamHandler()]
)

logger = logging.getLogger(__name__)


def analyze_roi_impact(db_path, print_miners=False):
    """
    Analyze the impact of ROI-based tier demotion on existing miners.
    
    Args:
        db_path (str): Path to the SQLite database
        print_miners (bool): Whether to print individual miner information
        
    Returns:
        dict: Statistics about how many miners would be demoted
    """
    logger.info(f"Analyzing ROI impact using database at {db_path}")
    
    conn = None
    try:
        # Connect to the database
        conn = sqlite3.connect(db_path)
        
        # Check if scores table exists
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='scores'")
        has_scores_table = cursor.fetchone() is not None
        
        if not has_scores_table:
            logger.error("Scores table not found in the database.")
            return None
        
        # Get the current date
        current_date = datetime.now(timezone.utc)
        
        # Get all active miners from miner_stats
        cursor.execute("""
            SELECT 
                miner_uid, 
                miner_current_tier,
                miner_current_roi
            FROM miner_stats
            WHERE miner_current_tier >= 3
            ORDER BY miner_current_tier DESC, miner_uid
        """)
        
        miners = cursor.fetchall()
        logger.info(f"Found {len(miners)} miners in tiers 3+")
        
        if not miners:
            logger.warning("No miners found in tiers 3 or above.")
            return None
        
        # Find the most recent ROI scores for each miner
        # We'll use the scores table where score_type='daily' and look for roi_score
        cursor.execute("""
            SELECT 
                s.miner_uid,
                s.day_id,
                s.roi_score
            FROM scores s
            WHERE s.score_type = 'daily'
            ORDER BY s.day_id DESC
        """)
        
        scores = cursor.fetchall()
        
        if not scores:
            logger.warning("No scores found.")
            return None
        
        # Convert to numpy array for easier processing
        scores_array = np.array(scores, dtype=np.float64)
        
        # Get most recent 15 days of scores for each miner
        miner_roi_data = {}
        
        # Get unique days sorted in descending order (most recent first)
        unique_days = np.unique(scores_array[:, 1])
        days = np.sort(unique_days)[::-1]  # Sort descending
        
        # Only take the most recent 15 days
        recent_days = days[:15] if len(days) > 15 else days
        
        # Process each miner's data
        unique_miners = np.unique(scores_array[:, 0]).astype(int)
        
        for miner_uid in unique_miners:
            # Get scores for this miner
            miner_mask = scores_array[:, 0] == miner_uid
            miner_scores = scores_array[miner_mask]
            
            # Filter to just recent days
            recent_mask = np.isin(miner_scores[:, 1], recent_days)
            recent_miner_scores = miner_scores[recent_mask]
            
            if len(recent_miner_scores) > 0:
                # Extract ROI values for recent days
                roi_values = recent_miner_scores[:, 2]
                miner_roi_data[miner_uid] = roi_values
        
        # Track demotions by tier
        tier_counts = {3: 0, 4: 0, 5: 0, 6: 0}
        tier_demotions = {3: 0, 4: 0, 5: 0, 6: 0}
        
        # Analyze each miner
        negative_roi_miners = []
        
        for miner_uid, tier, current_roi in miners:
            tier = int(tier)
            miner_uid = int(miner_uid)
            tier_counts[tier] = tier_counts.get(tier, 0) + 1
            
            # Check if we have ROI data for this miner
            if miner_uid in miner_roi_data:
                roi_values = miner_roi_data[miner_uid]
                
                # Only consider miners with enough history (5+ days)
                if len(roi_values) >= 5:
                    avg_roi = np.mean(roi_values)
                    
                    # Check if miner would be demoted
                    if avg_roi < 0:
                        tier_demotions[tier] = tier_demotions.get(tier, 0) + 1
                        negative_roi_miners.append((miner_uid, tier, avg_roi, len(roi_values)))
            
            elif print_miners:
                logger.info(f"No ROI data for miner {miner_uid} (tier {tier})")
        
        # Print detailed information about miners with negative ROI
        if print_miners and negative_roi_miners:
            logger.info("\nMiners with negative ROI that would be demoted:")
            for miner_uid, tier, avg_roi, num_days in sorted(negative_roi_miners, key=lambda x: x[2]):
                logger.info(f"Miner {miner_uid} (Tier {tier}): {avg_roi:.4f} ROI over {num_days} days")
        
        # Calculate percentage of miners affected
        tier_percentages = {}
        for tier in tier_counts.keys():
            if tier_counts[tier] > 0:
                percentage = (tier_demotions[tier] / tier_counts[tier]) * 100
                tier_percentages[tier] = percentage
            else:
                tier_percentages[tier] = 0
        
        # Print summary statistics
        logger.info("\nImpact Analysis Summary:")
        logger.info("------------------------")
        logger.info("Tier | Total Miners | Demotions | Percentage")
        logger.info("-----------------------------------------")
        for tier in sorted(tier_counts.keys()):
            logger.info(f"  {tier}  |     {tier_counts[tier]:<9} |    {tier_demotions[tier]:<6} |   {tier_percentages[tier]:.1f}%")
        
        total_miners = sum(tier_counts.values())
        total_demotions = sum(tier_demotions.values())
        overall_percentage = (total_demotions / total_miners) * 100 if total_miners > 0 else 0
        
        logger.info("-----------------------------------------")
        logger.info(f"Total |     {total_miners:<9} |    {total_demotions:<6} |   {overall_percentage:.1f}%")
        
        return {
            'total_miners': total_miners,
            'total_demotions': total_demotions,
            'tier_counts': tier_counts,
            'tier_demotions': tier_demotions,
            'tier_percentages': tier_percentages,
            'overall_percentage': overall_percentage,
            'negative_roi_miners': negative_roi_miners
        }
        
    except sqlite3.Error as e:
        logger.error(f"SQLite error: {e}")
        return None
    except Exception as e:
        logger.error(f"Error analyzing ROI impact: {e}", exc_info=True)
        return None
    finally:
        if conn:
            conn.close()


def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Analyze impact of ROI-based tier demotion")
    parser.add_argument(
        "--db-path", 
        type=str, 
        default="~/.bettensor/database.db",
        help="Path to SQLite database file (default: ~/.bettensor/database.db)"
    )
    parser.add_argument(
        "--print-miners",
        action="store_true",
        help="Print information about individual miners that would be demoted"
    )
    
    args = parser.parse_args()
    
    # Expand user path if necessary
    db_path = os.path.expanduser(args.db_path)
    
    if not os.path.exists(db_path):
        logger.error(f"Database file not found: {db_path}")
        sys.exit(1)
    
    try:
        analyze_roi_impact(db_path, args.print_miners)
    except Exception as e:
        logger.error(f"Error during analysis: {e}", exc_info=True)
        sys.exit(1)


if __name__ == "__main__":
    main() 